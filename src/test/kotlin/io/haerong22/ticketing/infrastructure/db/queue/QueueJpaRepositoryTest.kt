...
class QueueJpaRepositoryTest(
    ...
    fun `대기 순번을 조회 할 수 있다`(queueId: Long, expected: Long) {
        // given
        val queue1 = QueueEntity(token = UUID.randomUUID().toString(), status = TokenStatus.PROCEEDING)
        val queue2 = QueueEntity(token = UUID.randomUUID().toString(), status = TokenStatus.WAITING)
        val queue3 = QueueEntity(token = UUID.randomUUID().toString(), status = TokenStatus.WAITING)
        val queue4 = QueueEntity(token = UUID.randomUUID().toString(), status = TokenStatus.WAITING)
        val queue5 = QueueEntity(token = UUID.randomUUID().toString(), status = TokenStatus.WAITING)
